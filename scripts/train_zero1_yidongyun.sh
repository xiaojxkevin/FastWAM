...
echo "[launch] torchrun nproc_per_node=${NPROC_PER_NODE} nnodes=${NUM_MACHINES} node_rank=${MACHINE_RANK} rdzv_endpoint=${MAIN_PROCESS_IP}:${MAIN_PROCESS_PORT} run_id=${RUN_ID}"
